...
def generate_prompt(choice: int, place: str, activity: str = None):
  if choice == 1: # Function logic for activity recommendation
        return f"Generate a list of must try activities in {place}. Make sure to include the address of the location and star rating. Style this list as a text document"
  elif choice == 2: # Function logic for food recommendation
        return f"Generate a list of must try food spots in {place}. Make sure to include the address of the location and star rating. Style this list as a text document"
  elif choice == 3: #  Function logic for trip planner recommendation
        return f"Generate a list of must try {activity} in {place}. Make sure to include the address of the location and star rating. Style this list as a text document"
  elif choice == 4: # Function logic for custom recommendation
...
def test_model():
    ...
    try:
        ...
        prompt = f"Generate a list of 5 must try activities in {place}. Make sure to include a breif description, the address of the location, and star rating. Style this list as a text document but do not write any annotations explaining the styling"
